...
> Модель дерево решений, модель случайный лес. Как считается важность признаков в дереве

**Ответ:**

**Дерево решений:** рекурсивные разбиения пространства признаков.
...
**Случайный лес (Bagging):** ансамбль деревьев на бутстрап-подвыборках с случайным подмножеством признаков в узлах. Уменьшает дисперсию, устойчив к шуму.
...
Замечание: MDI склонна завышать важность признаков с многими уникальными значениями.

## Вопрос №20

> Ансамблирование алгоритмов. Бустинг. Другие алгоритмы понятно описаны тут и тут

**Ответ:**

**Ансамблирование:** комбинируем несколько моделей для снижения ошибки.

- **Bagging:** параллельные независимые базовые модели на бутстрап-выборках, усреднение/голосование (пример: Random Forest).

- **Stacking:** метамодель обучается на предсказаниях базовых.

- **Бустинг:** последовательные слабые модели исправляют ошибки предыдущих.

**Бустинг:**

- **AdaBoost:** взвешивает объекты. Ошибочные получают больший вес.

- **Gradient Boosting:** обучаем базовые модели на антиградиенте потерь.

- Производные: **XGBoost**, **LightGBM**, **CatBoost**.

**Плюсы:** высокая точность, устойчивость к разнородным данным.

**Минусы:** риск переобучения, чувствительность к гиперпараметрам, вычислительная стоимость.

**Ключевые гиперпараметры бустинга:** число деревьев, learning rate, глубина/листья, минимальный размер листа, регуляризация.
